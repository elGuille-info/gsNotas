...
//
// Usando System.Net.Http.HttpClient                                (19/oct/22)
//      que dice que System.Net.WebRequest.Create está obsoleto.
//      SYSLIB0014 'WebRequest.Create(string)' is obsolete: 'WebRequest, HttpWebRequest, ServicePoint, and WebClient are obsolete. Use HttpClient instead.'
//
...
namespace gsNotas
{
    public class VersionUtilidades
    {
        ...
        private static async Task<string> LeerContenidoWebAsync()
        {
            return await ClienteHttp.GetStringAsync(laUrl);
        }
        ...
        public static string VersionWeb(string aplicacion)
        {
            try
            {
                ...
                //string s = await ClienteHttp.GetStringAsync(laUrl);
                string s = Task.Run( async () => await LeerContenidoWebAsync()).Result;
        ...
        public static int CompararVersionWeb(Assembly ensamblado, ref string laVersionWeb)
        {
            ...
            var res = CompararVersionWeb(fvi.ProductName, fvi.FileVersion, ref laVersionWeb);
